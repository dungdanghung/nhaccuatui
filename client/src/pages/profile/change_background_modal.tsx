...
import { Setbackground } from "../../api/user";

export default function Change_background_modal() {
    const background_image = useRef<HTMLDivElement>(null)
    const select_image = useRef<HTMLImageElement>(null)
    const [value_hander_background_x, set_value_hander_background_x] = useState(0);
    const [value_hander_background_y, set_value_hander_background_y] = useState(0);
    ...
    function move_image_background(e: any) {
    ...
    function mouse_up_background() {
        window.removeEventListener('mousemove', move_image_background)
        window.removeEventListener('mouseup', mouse_up_background)
    }

    function click_image_select(e: any) {
        const element = e.target as HTMLDivElement
        set_value_hander_background_x(e.clientX)
        set_value_hander_background_y(e.clientY)
        if (element.className == "wrap_image_select") {
            window.addEventListener('mousemove', move_image_background)
            window.addEventListener('mouseup', mouse_up_background)
        }
    }

    function save() {
        const input = document.querySelector('.background_file') as HTMLInputElement
    ...
    return (
        <div className="change_background_form">
            <div className="change_background_header">
                <div className="change_background_header_title">Thay đổi ảnh nền</div>
